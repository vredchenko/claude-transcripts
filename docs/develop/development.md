...
| `config/` | Non-secret config — `config.template.json` → `config.json` (names, feature flags); see [configuration.md](../start/configuration.md). |
...
are mirrored to GHCR ([ADR 0024](../design/decisions/0024-mirror-backing-images-to-registry.md)).
