...
- **CouchDB doc schemas defined in code** + the self-built **migration** tool
  ([couchdb.md](../reference/couchdb.md), [migrations.md](../operate/migrations.md)).
- **webapi scaffold** — Bun + Hono + OpenAPI (the I/O gateway) ([webapi.md](../reference/webapi.md)).
- **CLI scaffold** — Bun + Ink, an aggregate of internal modules (generated webapi
  client, `.claude/` reader, hooks-setup, import/export) ([cli.md](../reference/cli.md)).
- **webui** — build forward from the current React + MUI SPA (router, central
  state, virtual scroll/lazy-load, generated client) ([webui.md](../reference/webui.md)).
- **Dev automation** — `scripts/` (orval client generation → CLI + SPA), run
  locally and wrapped in CI ([dev-automation.md](../develop/dev-automation.md)).
...
- **Claude Code compatibility** definition + the `latestPublic`/hooks generator;
  the **master hook table** ([compatibility.md](../start/compatibility.md), [hooks.md](../reference/hooks.md)).
- Work on the single **`main`** branch ([branching.md](../develop/branching.md)).

This is the tier the repo targets first. See
[specification.md](specification.md#7-installation) and [hook-setup.md](../start/hook-setup.md).
...
active-use features are built on solid ground. Design in [testing.md](../develop/testing.md).
...
- **Session export** — export a session (or a selection) to **PDF / Markdown /
  ...
  [migrations](../operate/migrations.md) machinery).
...
| App-log DB | 1/2 | optional | lose centralised app/error logs (see [app-logging.md](../operate/app-logging.md)) |
