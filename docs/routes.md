...
| `/api/sessions/{id}/turns` | **Speaker-split turns (per session)** | One side of the conversation (`?role=user`/`assistant`/…) or all turns, from `speaker_split/by_role` over full-content chunks ([ADR 0027](decisions/0027-full-content-chunks-in-couchdb.md)). Empty for sessions logged without `couchFullContentChunks`. |
...
| `/api/search` | **Full-text search** | `?q=…` over the Meilisearch `sessions` index (session metadata: cwd, model, tools, host). Best-effort — returns `enabled: false` + no hits when Meili is disabled/unreachable. Gated by `features.meilisearch` ([ADR 0009](decisions/0009-meilisearch-search.md)). |
