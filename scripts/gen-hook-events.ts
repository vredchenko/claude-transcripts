...
/**
 * Generate docs/reference/hook-events.md from the app model — the hook-events table is a
...
await Bun.write(join(ROOT, "docs", "reference", "hook-events.md"), doc);
console.log(
  `[gen-hook-events] wrote docs/reference/hook-events.md (${rows.length} events, ${wired.length} wired)`,
);
